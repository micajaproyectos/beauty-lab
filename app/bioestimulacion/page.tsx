import type { Metadata } from "next";
import Link from "next/link";
import CTA from "../components/CTA";
import Footer from "../components/Footer";

export const metadata: Metadata = {
  title: "Bioestimulación Facial en Santiago — Beauty Lab",
  description:
    "Bioestimulación facial para mejorar la calidad de la piel, estimular colágeno y recuperar luminosidad. Médico cirujana y master inyector certificada en Santiago, Chile.",
};

const beneficios = [
  "Estimula el colágeno propio del cuerpo",
  "Mejora la calidad, textura y luminosidad de la piel",
  "Resultados progresivos y naturales",
  "Trata flacidez incipiente sin cirugía",
  "Efecto que dura entre 12 y 18 meses",
  "Complementa y potencia otros tratamientos",
];

const proceso = [
  { paso: "01", titulo: "Diagnóstico", descripcion: "Evaluación de la calidad de la piel, grado de flacidez y expectativas para diseñar el protocolo adecuado." },
  { paso: "02", titulo: "Preparación", descripcion: "Limpieza de la zona y aplicación de anestesia tópica para garantizar el confort durante el procedimiento." },
  { paso: "03", titulo: "Aplicación", descripcion: "Microinyecciones del bioestimulador en las zonas definidas, distribuidas de forma uniforme para un resultado homogéneo." },
  { paso: "04", titulo: "Seguimiento", descripcion: "La piel mejora progresivamente durante semanas. Se pueden realizar sesiones de mantenimiento para potenciar los resultados." },
];

const faqs = [
  {
    pregunta: "¿Cuál es la diferencia entre bioestimulación y relleno con ácido hialurónico?",
    respuesta:
      "El ácido hialurónico rellena y repone volumen de forma directa. La bioestimulación, en cambio, no añade volumen sino que activa los fibroblastos de la piel para que produzcan su propio colágeno. Los resultados son más progresivos pero trabajan sobre la causa subyacente del envejecimiento cutáneo.",
  },
  {
    pregunta: "¿Cuántas sesiones se necesitan?",
    respuesta:
      "Depende del producto utilizado y el estado de la piel. Algunos bioestimuladores requieren una sola sesión con resultados que duran más de un año; otros protocolos recomiendan 2 o 3 sesiones iniciales separadas por 4 semanas.",
  },
  {
    pregunta: "¿Cuándo se ven los resultados?",
    respuesta:
      "La bioestimulación actúa de forma progresiva. Los primeros cambios se aprecian entre las 4 y 6 semanas, con la mejora máxima visible a los 2 o 3 meses cuando la neocolagénesis está activa.",
  },
  {
    pregunta: "¿Es compatible con otros tratamientos?",
    respuesta:
      "Sí. La bioestimulación se combina muy bien con toxina botulínica, ácido hialurónico e hilos tensores. Muchas veces forma parte de un protocolo integral de rejuvenecimiento.",
  },
  {
    pregunta: "¿Tiene tiempo de recuperación?",
    respuesta:
      "Puede haber algo de enrojecimiento o leve inflamación en las primeras 24 a 48 horas. La mayoría de las pacientes retoman sus actividades normales el mismo día.",
  },
];

export default function BioestimulacionPage() {
  return (
    <>
      <main>
        {/* Hero */}
        <div className="relative overflow-hidden bg-[#5C3A48] pt-32 pb-16">
          <div className="pointer-events-none absolute inset-0">
            <div className="absolute -top-24 -right-24 h-[500px] w-[500px] rounded-full bg-[#8B5E6D]/30 blur-3xl" />
            <div className="absolute -bottom-24 -left-24 h-[400px] w-[400px] rounded-full bg-[#2C2329]/40 blur-3xl" />
          </div>
          <div className="relative mx-auto max-w-7xl px-6 lg:px-12">
            <nav className="mb-8 flex items-center gap-2" aria-label="Breadcrumb">
              <Link href="/" className="font-[family-name:var(--font-inter)] text-xs font-light tracking-wider text-[#E8C9C1]/60 uppercase transition-colors hover:text-[#E8C9C1]">Inicio</Link>
              <span className="text-[#E8C9C1]/40">/</span>
              <Link href="/rejuvenecimiento-facial" className="font-[family-name:var(--font-inter)] text-xs font-light tracking-wider text-[#E8C9C1]/60 uppercase transition-colors hover:text-[#E8C9C1]">Rejuvenecimiento Facial</Link>
              <span className="text-[#E8C9C1]/40">/</span>
              <span className="font-[family-name:var(--font-inter)] text-xs font-light tracking-wider text-[#C9A96E] uppercase">Bioestimulación</span>
            </nav>
            <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
              <div>
                <span className="inline-block font-[family-name:var(--font-inter)] text-xs font-medium tracking-[0.3em] text-[#C9A96E] uppercase mb-3">Regeneración cutánea</span>
                <h1 className="font-[family-name:var(--font-cormorant)] text-5xl font-light leading-[1.1] text-[#FAF8F5] lg:text-6xl">
                  Bioestimulación
                  <br />
                  <em className="text-[#E8C9C1]">facial</em>
                </h1>
              </div>
              <p className="max-w-sm font-[family-name:var(--font-inter)] text-sm font-light leading-relaxed text-[#E8C9C1]/70">
                Activa la producción natural de colágeno para recuperar la firmeza, luminosidad y calidad de tu piel desde adentro.
              </p>
            </div>
            <div className="mt-10 flex flex-wrap items-center gap-8 border-t border-white/10 pt-8">
              {[
                { value: "30–60 min", label: "Duración sesión" },
                { value: "4–6 semanas", label: "Primeros resultados" },
                { value: "12–18 meses", label: "Duración del efecto" },
                { value: "Progresivo", label: "Mejora continua" },
              ].map((s) => (
                <div key={s.label} className="flex items-baseline gap-1.5">
                  <span className="font-[family-name:var(--font-cormorant)] text-2xl font-light text-[#C9A96E]">{s.value}</span>
                  <span className="font-[family-name:var(--font-inter)] text-xs font-light tracking-wider text-[#E8C9C1]/60">{s.label}</span>
                </div>
              ))}
            </div>
          </div>
        </div>

        {/* Qué es */}
        <section className="bg-[#FAF8F5] py-20 lg:py-28">
          <div className="mx-auto max-w-7xl px-6 lg:px-12">
            <div className="grid gap-16 lg:grid-cols-2 lg:gap-24 items-center">
              <div>
                <span className="inline-block font-[family-name:var(--font-inter)] text-xs font-medium tracking-[0.3em] text-[#C9A96E] uppercase mb-4">¿Qué es?</span>
                <h2 className="font-[family-name:var(--font-cormorant)] text-4xl font-light leading-[1.15] text-[#2C2329] mb-6 lg:text-5xl">
                  Tu piel produce
                  <br />
                  <em className="text-[#5C3A48]">su propio colágeno</em>
                </h2>
                <p className="font-[family-name:var(--font-inter)] text-base font-light leading-relaxed text-[#7A6A6E] mb-4">
                  La bioestimulación es un tratamiento médico que consiste en la aplicación intradérmica de sustancias bioestimulantes — como Radiesse, Sculptra o Profhilo — que activan los fibroblastos para producir colágeno, elastina y ácido hialurónico propios.
                </p>
                <p className="font-[family-name:var(--font-inter)] text-base font-light leading-relaxed text-[#7A6A6E]">
                  A diferencia del relleno convencional, no aporta volumen externo sino que trabaja sobre los mecanismos biológicos del envejecimiento, mejorando la calidad intrínseca de la piel de forma progresiva y duradera.
                </p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                {beneficios.map((b) => (
                  <div key={b} className="flex items-start gap-3 rounded-2xl border border-[#E8C9C1]/60 bg-white p-5">
                    <span className="mt-0.5 flex-shrink-0 text-[#C9A96E]">✓</span>
                    <span className="font-[family-name:var(--font-inter)] text-sm font-light leading-snug text-[#7A6A6E]">{b}</span>
                  </div>
                ))}
              </div>
            </div>
          </div>
        </section>

        {/* Proceso */}
        <section className="bg-white py-20 lg:py-28">
          <div className="mx-auto max-w-7xl px-6 lg:px-12">
            <div className="mb-14 text-center">
              <span className="inline-block font-[family-name:var(--font-inter)] text-xs font-medium tracking-[0.3em] text-[#C9A96E] uppercase mb-4">Proceso</span>
              <h2 className="font-[family-name:var(--font-cormorant)] text-4xl font-light text-[#2C2329] lg:text-5xl">
                ¿Cómo es la
                <br />
                <em className="text-[#5C3A48]">experiencia?</em>
              </h2>
            </div>
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {proceso.map((p) => (
                <div key={p.paso} className="flex flex-col rounded-2xl border border-[#E8C9C1]/60 bg-[#FAF8F5] p-6">
                  <span className="mb-4 font-[family-name:var(--font-cormorant)] text-4xl font-light text-[#D4A5A0]">{p.paso}</span>
                  <div className="mb-3 h-px w-8 bg-[#C9A96E]" />
                  <h3 className="mb-2 font-[family-name:var(--font-cormorant)] text-xl font-medium text-[#2C2329]">{p.titulo}</h3>
                  <p className="font-[family-name:var(--font-inter)] text-sm font-light leading-relaxed text-[#7A6A6E]">{p.descripcion}</p>
                </div>
              ))}
            </div>
            <div className="mt-12 text-center">
              <Link href="/servicios" className="inline-flex items-center gap-2 rounded-full bg-[#5C3A48] px-8 py-3.5 font-[family-name:var(--font-inter)] text-sm font-medium tracking-widest text-[#FAF8F5] uppercase shadow-[0_8px_32px_rgba(92,58,72,0.3)] transition-all hover:bg-[#2C2329]">
                Cotizar tratamiento
                <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M17.25 8.25L21 12m0 0l-3.75 3.75M21 12H3" />
                </svg>
              </Link>
            </div>
          </div>
        </section>

        {/* FAQ */}
        <section className="bg-[#FAF8F5] py-20 lg:py-28">
          <div className="mx-auto max-w-3xl px-6 lg:px-12">
            <div className="mb-14 text-center">
              <span className="inline-block font-[family-name:var(--font-inter)] text-xs font-medium tracking-[0.3em] text-[#C9A96E] uppercase mb-4">Preguntas frecuentes</span>
              <h2 className="font-[family-name:var(--font-cormorant)] text-4xl font-light text-[#2C2329] lg:text-5xl">
                Todo lo que
                <br />
                <em className="text-[#5C3A48]">necesitás saber</em>
              </h2>
            </div>
            <div className="space-y-4">
              {faqs.map((f) => (
                <div key={f.pregunta} className="rounded-2xl border border-[#E8C9C1]/60 bg-white p-6 lg:p-8">
                  <h3 className="mb-3 font-[family-name:var(--font-cormorant)] text-xl font-medium text-[#2C2329]">{f.pregunta}</h3>
                  <p className="font-[family-name:var(--font-inter)] text-sm font-light leading-relaxed text-[#7A6A6E]">{f.respuesta}</p>
                </div>
              ))}
            </div>
          </div>
        </section>

        <CTA />
      </main>
      <Footer />
    </>
  );
}
